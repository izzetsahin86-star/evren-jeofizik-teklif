(() => {
  ...
  const META_KEY = "evren-controlled-sync-meta-v1";
  const LAST_SYNC_KEY = "evren-controlled-sync-last-v1";
  ...
  function itemKey(item, index) {
    return String(item?.id || item?.no || `index-${index}`);
  }
  ...
  function updateVisibility() {
    ...
    if (!panel) return;
    panel.hidden = sessionStorage.getItem(AUTH_KEY) !== "1";
  }
  ...
  function mergeObjectArray(localItems, remoteItems, baselineMap = {}) {
    ...
    keys.forEach((key) => {
      ...
      const localChanged = base ? lh !== base : false;
      const remoteChanged = base ? rh !== base : true;

      if (base && localChanged && !remoteChanged) {
        merged.push(l);
        return;
      }
      if (base && remoteChanged && !localChanged) {
        merged.push(r);
        return;
      }
      ...
      if (lTime || rTime) merged.push(lTime >= rTime ? l : r);
        ...
        // Aynı kayıt iki cihazda farklıysa mevcut cihazdaki sürümü koru;
        // hiçbir kaydı silme ve kullanıcıya çakışma olduğunu bildir.
        merged.push(l);
        conflicts += 1;
      }
    });
  ...
  function mergeStates(localState, remoteState, baseline = {}) {
    ...
    for (const key of ["quotes", "customers", "companies", "services"]) {
      const merged = mergeObjectArray(local[key], remote[key], baseline?.[key]);
  ...
  async function fullSync({ reason = "manual", allowReload = false } = {}) {
    ...
    try {
      ...
      if (!current.response.ok || !current.payload?.ok || !current.payload?.initialized) {
      ...
      const after = stable(merged.state);
      ...
      if (allowReload && before !== after) {
        // Yalnızca kullanıcının manuel senkronunda veya girişte tek seferlik yenileme.
        // Saatlik senkron hiçbir zaman sayfayı yenilemez.
  ...
  async function hourlySync() {
    ...
    try {
      ...
      const baselineHash = hashValue(meta.baseline || {});
      const currentBaselineHash = hashValue(baselineFor(local));
      if (knownRevision && baselineHash === currentBaselineHash) {
      ...
      // Saatlik kontrol sessizdir; sistemi kullanmayı engellemez.
    }
  }
